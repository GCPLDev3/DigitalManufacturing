...
gj.widget.prototype.reload = function (params) {
    ...
    if ($("#dvKPIs").length) {
        var additional =  {
            masterParameterId: Number($("#hdnMasterParameterId").val())
        }
        $.extend(data.params, additional);
    }

    if ($("#dvParameterGrid").length) {
        var dateIsoString = $("#ddlAuditDate").val() ? new Date($("#ddlAuditDate").val()) : null;
        var additional = {
            isDefault: $("#hdnIsDefault").val().toLowerCase() === "true" ? true : false,
            isUpload: $("#hdnIsUpload").val().toLowerCase() === "true" ? true : false,
            scheduleId: $("#ddlSchedule").val(),
            masterParameterId: Number($("#MasterParameterId").val())
        }
        $.extend(data.params, additional);
    }
    if ($("#dvScoreBoard").length || $("#dvIRAPerformance").length) {
        var additional = {
            scheduleId: $("#ddlSchedule").val()
        }
        $.extend(data.params, additional);
    ...
    if ($("#dvIRADashBoard").length) {
        if ($("#specificIRA").length) {
            var additional = {
        ...
        } else if ($("#specificCompetency").length) {
            var additional = {
                fromDate: $('#txtFromDate').val() ? $('#txtFromDate').datepicker('getDate').toISOString() : "",
                toDate: $('#txtToDate').val() ? $('#txtToDate').datepicker('getDate').toISOString() : "",
                personId: $("#hdnPersonId").val(),
...
gj.grid.methods = {
            ...
            data.columns.splice(gj.grid.methods.getColumnPositionNotInRole($grid), 0, {
                ...
                events: {
                    click: function (e) {
                        ...
                        var isChecked = true;
                        $("#grid tbody tr :checkbox").each(function () {
                            if (this.checked == false)
                                isChecked = false;
                        });

                        $('.gj-grid-select-all').find(':checkbox').each(function () {
    ...
    renderCell: function ($grid, $cell, column, record, id, mode) {
        var $displayEl, key;
        if (!$cell || $cell.length === 0 || column.type === "renderer") {
            $cell = $('<td data-name= "' + column.field + '"/>').css('text-align', column.align || 'left');
            $displayEl = $('<div data-role="display" />');
            if (column.role === "selectRow" && column.type === "checkbox") {
                $displayEl = $('<input type="checkbox" data-role="selectAll" />');
            }
          
            if (column.cssClass) {
                $cell.addClass(column.cssClass);
            }
            if (column.type === "renderer") {
                $cell.html("");
...
gj.grid.plugins.inlineEditing.private = {
            ...
            if ($editorField.length) {
                ...
                if (typeof (column.editor) === 'function') {
                    ...
                    if ('checkbox' === column.type) {
                        ...
                        if (column.field === "ScoreObtained") {
                            $editorContainer.append('<input type="number" min="0" max="' + record.MaximumScore + '" value="' + value + '" class="gj-width-full "/>');
                        } else {
                            $editorContainer.append('<input type="number"  value="' + value + '" class="gj-width-full "/>');
                        }
                    }
                    else if ('dropdown' === column.type) {
                        if (column.field === "Reminder") {
                            var list = [
                                "Yes",
                                "No",
                                 "N/A"
                            ...
                            var stringHtml=""
                            stringHtml="<div id='country'><select class='form-control select2'>";
                            $.each(list, function (id, Value) {
                                if (Value === value) {
                                    stringHtml= stringHtml.concat("<option  value=" + Value + " selected>" + Value + "</option>");
                                } else {
                                    stringHtml= stringHtml.concat("<option  value=" + Value + ">" + Value + "</option>");
                                }
      
                            });
                            stringHtml=stringHtml.concat("</select></div>");
                            $editorContainer.append(stringHtml);
                        }

                        //$editorContainer.append("<div id='remainder'><select class='form-control select2 select2-hidden-accessible' id='ddlRemainder'  style='width: 100%;' tabindex='-1' aria-hidden='true'><option value='1'>Yes</option><option value='2'>No</option><option selected='selected' value='3'>N/A</option></select></div>");
                    } else if ('textArea' === column.type) {
                        $editorContainer.append('<textarea>' + value + '</textarea>');
                    }
                    else {
    ...
    displayMode: function ($grid, $cell, column, cancel) {
        ...
        if ($cell.attr('data-mode') === 'edit') {
            ...
            if (column.field === "ScoreObtained") {
                if (newValue > record.MaximumScore) {
                    //default:ScoreObtained is greater than maximum score then the ScoreObtained replaced to maximum score
                    newValue = record.MaximumScore;
                }
            }
